            ...
            {products.map((ele, ind) => (
                ele.name ? (
                    <div key={ind} className='bg-accent2 hover:bg-accent2/90 duration-500 border border-primary cursor-pointer p-10'>
                        <p className='text-primary font-bold font-neogrotesque text-3xl'>{ele.name}</p>
                        <p className='text-secondary font-medium font-neogrotesque text-lg'>{data}</p>
                        <img src={ele.image} height='auto' width='auto' alt={ele.name || "Product Image"} className="w-full h-auto rounded-md object-cover scale-75 hover:scale-100 duration-700" />
                    </div>
                ) : null
            ))}
        </div>
